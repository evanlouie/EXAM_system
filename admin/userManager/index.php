		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$(document).on('click', 'button.deleteUser', function() {
					if (confirm("Note: This will complete erase all information associated with this account; including personal and affiliated attempt information. Do you wish to continue?")) {
						user_id = $('input:hidden').val();
						$.get('callbacks.php?delete&user_id=' + user_id);
					}
				})
				$(document).on('click', 'button.deleteAttemptButton', function() {
					if (confirm("Are you sure you wish to delete attempt?")) {
						attempt_id = $(this).closest('tr').attr('id');
						$.get('callbacks.php?delete&attempt_id=' + attempt_id, function() {
							$('code#pastAttempts').load('callbacks.php?loadAttempts&user_id=' + user_id);
						});
				...
				$(document).on('click', 'button.deleteAllAttemptsButton', function() {
					if (confirm("are you sure you wish to delete ALL user attempts?")) {
						user_id = $('select#userList').val();
						$.get('callbacks.php?deleteAttempts&user_id=' + user_id, function() {
							$('code#pastAttempts').load('callbacks.php?loadAttempts&user_id=' + user_id);
						})
					}
				})
...
<!-- 				<button id='loadUser'>
				...
				<code id='userInfo'>
					Select user from menu above
				</code>

				<p>
					Past exam attempts:
				</p>
				<code id='pastAttempts'>
